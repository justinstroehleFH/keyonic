...
export class DetailsPage implements OnInit {
  ...
  private entry: Password = {id: '-1', label: [], password: "", title: "", url: "", username: ""}
  protected labels: any[] = ['Work', 'Uni', 'Shopping'];
  public newEntry = true;
  public detailForm!: FormGroup;

  constructor(private keyonicService: KeyonicService, private activatedRoute: ActivatedRoute, private formBuilder: FormBuilder, private router: Router) {}
  ...
  static matchPassword(form: AbstractControl){
    return form.get('password')?.value == form.get('repeat')?.value ? null : {equals: true};
  }
  ...
  protected generatePassword() {
    ...
    this.keyonicService.showToast('generiertes Passwort wurde in die Zwischenablage gespeichert', 1500, 'top');
  }

  public save() {
     if(!this.detailForm.valid){
      this.keyonicService.showToast('Du Hurensohn füll das Formular aus. Danke', 1500, 'top');
      return;
    }
    ...
    this.keyonicService.saveEntry(password);
